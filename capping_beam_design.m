...
% dimensions
N_max = 20;              % number of longitudinal bars - flexure - max
N_max_tor = 20;			% number of longitudinal bars - torsion - max
Rebar_dia = [6, 8, 10, 12, 16, 20, 25, 32, 40];     % unit: mm     % bar diameter - minimum  
stirrup_diameter = [8, 10, 12, 16, 20, 25];   % unit: mm
...
% M and V from the GSA analysis; T from column Myy loading assuming it is fixed
M_pos_load = 655.6;           % Moment+   unit: kNm
M_neg_load = 379.5;           % Moment-   unit: kNm
V_load = 228.6;               % Shear     unit: kN
T_load = 1352;                % Torsion   unit: kNm
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
d = depths-cover-(stirrup_reinforcements_chosen(1)/2);
As_min = max(0.26*(fctm/fyk)*widths*d, 0.0013*widths*d);
As_max = 0.04*depths*widths;

if rebar_areas_pos_chosen(5)+rebar_areas_neg_chosen(5)+rebar_areas_tor_chosen(5) < As_min 
    error('try different dimensions')
elseif rebar_areas_pos_chosen(5)+rebar_areas_neg_chosen(5)+rebar_areas_tor_chosen(5) > As_max
	error('try different dimensions')
end
