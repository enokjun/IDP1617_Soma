...
column_list = {'Riverside - outer', 'Riverside - inner'};

% Design load
capping_beam_D = 1*0.8*25;      
Dead_P_list = [1229, 415]+repmat([capping_beam_D],1,length(column_list));      % unit: kN
Live_P_list = [1060.3, 238.8];       % unit: kN
Wind_P_list = [406.4, -33.14];       % unit: kN

load_type_list = {'compression','tension'};

% soil
% [depth/m, thickness/m, bulk density/kNm^-3, Cu/kPa, friction angle, c/kPa, alpha, E/MPa, v, Cr, Cc]
soil_list = {'AA1_soil_profile.csv','AA1_soil_profile.csv'};
...
B = [0.6, 0.75, 0.9, 1.05, 1.2];  % unit: m
Lmin = 10;   % unit: m
Lmax = 30;   % unit: m
...
Mmin = 3;
Mmax = 8; 
Space = [0.9, 1.15, 1.35, 1.6, 1.8];  % unit: m
